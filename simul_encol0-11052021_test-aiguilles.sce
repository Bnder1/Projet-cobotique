...
if (ee<0.4) | (ee>0.8) then disp('Erreur : Déplacement seringue non valide'); return; end;   //  Déverrouiller les valeurs après le PE
if (xx<70) | (xx>70) then disp('Erreur : Déplacement non valide'); return; end; //  Déverrouiller les valeurs après le PE
if (Vx<200) | (Vx>600) then disp('Erreur : Vitesse non valide'); return; end; //  Déverrouiller les valeurs après le PE
if (hh<0.2) | (hh>0.8) then disp('Erreur : hauteur aiguille non valide'); return; end; //  Déverrouiller les valeurs après le PE

//Normalisation des valeur d'entrées pour qu'elles soient comprisent entre -1 et 1 (Plan d'expérience utilsant des matrise de Taguchi)
eeNorm = -1 +((2*(ee-eMin))/(eMax-eMin));
hhNorm = -1 +((2*(hh-hMin))/(hMax-hMin));
VxNorm = -1 +((2*(Vx-VxMin))/(VxMax-VxMin));

//Calcul des interactions de notre plan d'expérience 
eh= eeNorm*hhNorm;
eVx= eeNorm*VxNorm;
hVx= hhNorm*VxNorm;
ehVx= eeNorm*hhNorm*VxNorm;
...
TauVx = mean_tauVx + (4.3*eeNorm) + (-3.525*hhNorm) + (-4.65*VxNorm) + (3.55*eh) + (-3.275*eVx) + (6.55*hVx) + (-1.275*ehVx);
...
a0 = mean_a0 + (0.65*eeNorm) + (0.025*hhNorm) + (-0.175*VxNorm) + (-0.1*eh) + (-0.15*eVx) + (-0.025*hVx) + (0.1*ehVx); //Formule
